export class PrayerTimeUpdater {
    ...
    constructor() {
        ...
        this.get_data(
            this.curr_month, this.curr_year, this.month_of_data, this.year_of_data,this.current_month_data, 
            [this.fajr_offset, this.syuruk_offset, this.dhuhr_offset, this.asr_offset, this.maghrib_offset, this.isha_offset],
            this.get_new_month_data, this.renew_api_link);
    }    

    get_data(browser_month, browser_year, data_month, data_year, curr_data, offsets, get_new_data, renew_api) {
        // check month_validity, then get current or new data

        chrome.storage.sync.get(null, function (result) {
            data_month = result.curr_month_key;
            data_year = result.curr_year_key;
            console.log(`data_month = ${data_month}, browser_month = ${browser_month}`)
            if (browser_month != data_month | browser_year != data_year) {
                console.log(`Data is old. Retrieving new prayer times `)
                get_new_data(browser_month, browser_year, curr_data, offsets, renew_api);
            } else {
    ...
    get_new_month_data(month_query, year_query, data_query, offsets, renew_api) {
        ...
        renew_api(month_query, year_query, data_query, offsets, function (data_q, api_link){
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState == 4) {
                    const PRAYER_NAMES = ["Fajr", "Sunrise", "Dhuhr", "Asr",
                        "Maghrib", "Isha"];
                    ...
                    for (var k = 0; k < month_data.length; k++) {
                        ...
                        for (var prayer_idx = 0; prayer_idx < PRAYER_NAMES.length; prayer_idx++) {
                    ...
                    chrome.storage.sync.set({
                        ...
                        console.log(`Raw API data for ${month_query}/${year_query} ` +
    ...
    renew_api_link(month_q, year_q, data_q, offsets, callback) {
        ...
        chrome.storage.sync.get(null, function (result) {
            ...
            offsets[1]= ((result.syuruk_offset_key != null) ? result.syuruk_offset_key : 0);
            offsets[2]=  ((result.dhuhr_offset_key != null) ? result.dhuhr_offset_key : 0);
            offsets[3]= ((result.asr_offset_key != null) ? result.asr_offset_key : 0);
            offsets[4]=  ((result.maghrib_offset_key != null) ? result.maghrib_offset_key : 0);
            offsets[5]= ((result.isha_offset_key != null) ? result.isha_offset_key : 0);

            console.log(`renew_api -> chrome_storage -> offsets after result: ${offsets}`)

            var API_LINK = `https://api.aladhan.com/v1/calendarByCity?city=Singapore&country=Singapore` +
                        `&method=11&` +
                        `month=${month_q}` +
                        `&year=${year_q}` +
                        `&tune=0,` +
    ...
    update_offsets(fajr_offset, syuruk_offset, dhuhr_offset, asr_offset, maghrib_offset, isha_offset) {
        chrome.storage.sync.get(null, function (result) {

            fajr_offset = ((result.fajr_offset_key != null) ? result.fajr_offset_key : 0);
            syuruk_offset = ((result.syuruk_offset_key != null) ? result.syuruk_offset_key : 0);
            dhuhr_offset = ((result.dhuhr_offset_key != null) ? result.dhuhr_offset_key : 0);
            asr_offset = ((result.asr_offset_key != null) ? result.asr_offset_key : 0);
            maghrib_offset = ((result.maghrib_offset_key != null) ? result.maghrib_offset_key : 0);
            isha_offset = ((result.isha_offset_key != null) ? result.isha_offset_key : 0);

            console.log(`retrieved offsets from storage. fajr: ${fajr_offset} syuruk: ${syuruk_offset}`)
    ...
    calculate_adjusted_time(suggested_time, offset) {
        ...
        var offset_h = (offset < 0 ? Math.floor(offset / 60) + 1 : Math.floor(offset / 60));
